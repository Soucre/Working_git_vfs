            ...
            else
            {
                ...
                if (customerData != null)
                {
                    ...
                    foreach (CustomerStockInquiryObject obj in customerInquiryData)
                    {
                        ...
                        /*********************test***************************                     
                        customer.Email = "hieu.nguyen@vfs.com.vn";
                        customer.Mobile = "0909070481";
                        customer.Mobile2 = "0909070481";
                        customer.MobileSms = "0909070481";
                        /*********************test***************************/

                        Ultility.Info(customer.CustomerId + ": " + obj.StockCode);

                        /// lấy danh mục tin cổ phiếu ứng với cổ đông và cổ phiếu của họ
                        ...
                        stockNewsCollection = stock_NewService.Getstock_NewList(obj.StockCode, customer.CustomerId);
                        ...
                        foreach (stock_New stock_new in stockNewsCollection)
                        {
                            ...
                            //website cũ sử dụng cái này: content.Append("<td style=\"background-color:#F5F5F5;text-align:left;font:Tahoma;\"><a style=\"cursor:pointer; text-decoration:none;font:Tahoma;color:#000000;\" href=\"http://portal.vfs.com.vn/News.aspx?newsid=" + stock_new.NewsID.ToString() + "&group=" + SetNewsGroupName(stock_new.NewsID) + "\"" + ">" + (stock_new.NewsTitle.Normalize().IndexOf(':') == 3 ? stock_new.NewsTitle.Normalize().Substring(4) : stock_new.NewsTitle.Normalize().Substring(3)) + "</a></td>");
                            // website mới 
                            content.Append("<td style=\"background-color:#F5F5F5;text-align:left;font:Tahoma;\"><a style=\"cursor:pointer; text-decoration:none;font:Tahoma;color:#000000;\" href=\"http://www.vfs.com.vn/tabid/334/news/" + stock_new.NewsID.ToString() + "/tin-lien-quan-danh-muc-dau-tu.aspx" + "\"" + ">" + (stock_new.NewsTitle.Normalize().IndexOf(':') == 3 ? stock_new.NewsTitle.Normalize().Substring(4) : stock_new.NewsTitle.Normalize().Substring(3)) + "</a></td>");
                            content.Append("</tr>");
                            if (smsCount == 0)
                    ...
                    if (sentYn)
                    {
                        ...
                        {
                            // lưu vào log là đã gửi rồi
                            Vfs.WebCrawler.Destination.Business.RelatedMessagelogService.CreateRelatedMessagelog2(relatedMsglog); 
                        }
                        Ultility.Info(customer.CustomerId + ": " + "da gui");
                    }
                }
            }
        }
