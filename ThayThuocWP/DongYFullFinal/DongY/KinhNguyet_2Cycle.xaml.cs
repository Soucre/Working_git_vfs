...
namespace DongY
{
    public partial class KinhNguyet : PhoneApplicationPage
    {
        ...
        "<html>" +
            "<head>" +
                "<meta name=\"Viewport\" content=\"width=device-width; user-scaleable=yes; initial-scale=0.5; height=100\" />" +
                ...
                "}</script> " +
            "</head>" +
            "<body style=\"text-height:15px;margin:0px;padding:0px\" background =\"../Assets/background_320wx416h.png\" onLoad=\"SendDataToPhoneApp()\" >" +
                "<center>" +
                "<div id=\"chart\" style=\"zoom:60%\" >" +
                    "<table width=\"00\"  border=\"1\" style=\"text-align:center\">" +
                    "<tr>" +
                      "<td width=\"150\">Tháng</td>";

            DateTime EndDateCir2 = EndDate.AddDays(Circle);
            int EndDayCir2 = EndDateCir2.Day;
            int EndMonCir2 = EndDateCir2.Month;
            int EndYearCir2 = EndDateCir2.Year;

            if (BegMon == EndMonCir2)
            {
                result += "<td id=\"month_1\" colspan=\""+ (2*Circle).ToString() +"\">Tháng " + BegMon.ToString() + "</td>";
            }
            else
            {
                List<int> Month = new List<int>();
                while ((EndMonCir2 - BegMon) >= 0)
                {
                    Month.Add(BegMon);
                    BegMon++;
                }
                ...
                {
                    int j = ((DateTime.DaysInMonth(EndYearCir2, Month[i]) - sday + 1));
                    sday = 1;
                    result += "<td id=\"month_1\" colspan=\""+ j.ToString() +"\">Tháng " + Month[i].ToString() + "</td>";
                }
                result += "<td id=\"month_1\" colspan=\"" + (2*Circle - sday + 1).ToString() + "\">Tháng " + Month[Month.Count - 1].ToString() + "</td>";
            ...
            result += "</tr><tr><td height=\"100\">Ngày quan hệ tự do</td>";
            result += GetChart_Free(BeginDate, Circle, Number);
            result += GetChart_Free(EndDate.AddDays(1), Circle, Number);
            
            result += "</tr><tr><td height=\"100\">Khả năng có thai và rụng trứng</td>";
