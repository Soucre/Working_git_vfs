...
$result = $db->query("SELECT *, (select `title` from `category` where `category`.`category_id` = `product`.`category`) as `category` FROM `product` $condition $order_by LIMIT $start, $length");
$data = "";

$count_result = $db->query("SELECT COUNT(*) count FROM `product`");
$count_obj = $count_result->fetch_object();

$filter_count_result = $db->query("SELECT COUNT(*) count FROM `product` $condition");
