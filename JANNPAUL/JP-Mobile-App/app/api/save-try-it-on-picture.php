...
if ($_FILES["picture"]["name"] && $_FILES["picture"]["tmp_name"]) {
    ...
    if ($check) {
        ...
        if ($imageFileType != "png") {
            $file_msg .= "<br>Sorry, only PNG is allowed.";
            $uploadOk = false;
        }
...
if ($uploadOk && file_exists("../../" . $target_file)) {
    $set_statement .= "SET try_it_on_picture = '$target_file'";

// Save current
    $sql = "UPDATE `product_variant` $set_statement WHERE `variant_id` = " . filter_input(INPUT_POST, "variant_id");
