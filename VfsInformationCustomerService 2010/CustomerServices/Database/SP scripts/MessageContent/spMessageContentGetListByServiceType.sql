...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spMessageContentGetListByServiceType]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[spMessageContentGetListByServiceType]
GO
...
**    
*******************************************************************************/
CREATE PROCEDURE [dbo].spMessageContentGetListByServiceType
	@Status int,
	@ServiceType int,
	@OrderBy varchar(50),
...
SET @sql = @sql + ' [MessageContentID] FROM [dbo].[MessageContent] WHERE Status =' + Convert( nvarchar(10), @Status) + ' AND [ServiceTypeID] =' + Convert( nvarchar(10), @ServiceType) + ' ORDER BY [' + @OrderBy + N'] ' + @OrderDirection
EXEC (@sql)
SET ROWCOUNT 0
...
AND Status = @Status AND [ServiceTypeID] = @ServiceType
ORDER BY RowNumber
