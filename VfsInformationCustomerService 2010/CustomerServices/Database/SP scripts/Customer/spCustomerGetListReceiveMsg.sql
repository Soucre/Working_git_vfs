...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spCustomerGetListReceiveMsg]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[spCustomerGetListReceiveMsg]
GO
...
**    
*******************************************************************************/
CREATE PROCEDURE [dbo].spCustomerGetListReceiveMsg
	@OrderBy varchar(50),
...
SET @sql = @sql + ' [CustomerId] FROM [dbo].[Customer] WHERE (ReceiveRelatedStockEmail = ''Y'' OR ReceiveRelatedStockSms = ''Y'')  ORDER BY [' + @OrderBy + N'] ' + @OrderDirection
EXEC (@sql)
SET ROWCOUNT 0

SELECT @TotalRecords = COUNT(*) FROM [Customer] WHERE ReceiveRelatedStockEmail = 'Y' OR ReceiveRelatedStockSms = 'Y'
...
WHERE (@PageSize = 0) OR (@PageSize > 0 AND (@Page - 1)*@PageSize < RowNumber AND RowNumber <= @Page*@PageSize)
        AND (ReceiveRelatedStockEmail = 'Y' OR ReceiveRelatedStockSms = 'Y')
ORDER BY RowNumber
